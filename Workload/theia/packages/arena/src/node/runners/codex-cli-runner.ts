...
@injectable()
export class CodexCliArenaRunner implements IArenaRunner {

    readonly info = {
        id: 'codex-cli',
        name: nls.localize('theia/arena/runner/codexProvider/name', 'Codex CLI Runner'),
        description: nls.localize('theia/arena/runner/codexProvider/description', 'Runs Codex CLI non-interactively inside the candidate sandbox.'),
        available: true,
        capabilities: {
    ...
    async run(request: ArenaRunRequest, cancellationToken?: ArenaRunnerCancellationToken): Promise<ArenaRunResult> {
        ...
        try {
            ...
            if (unexpected.length) {
                ...
                artifact.error = nls.localize('theia/arena/codexChangedOutsideOutput', 'Codex CLI changed files outside output/: {0}', unexpected.join(', '));
            }
            return {
                ...
                error: result.exitCode === 0 ? artifact.error ?? undefined : nls.localize('theia/arena/codexExitedWithCode', 'Codex CLI exited with code {0}', result.exitCode)
            };
        } catch (error) {
...
- Não altere arquivos fora de output/, exceto arquivos temporários internos indispensáveis.`;
    ...
    protected spawnCodex(
        ...
        return new Promise((resolve, reject) => {
            if (cancellationToken?.isCancellationRequested) {
                reject(new Error(nls.localize('theia/arena/codexRunCancelled', 'Codex CLI run cancelled.')));
                return;
            }
            ...
            const timer = setTimeout(() => {
                failureReason = new Error(nls.localize('theia/arena/codexTimedOut', 'Codex CLI timed out after {0}s.', CODEX_TIMEOUT_MS / 1000));
                this.killProcessTree(child.pid);
            }, CODEX_TIMEOUT_MS);
            cancellationToken?.onCancellationRequested(() => {
                failureReason = new Error(nls.localize('theia/arena/codexRunCancelled', 'Codex CLI run cancelled.'));
                this.killProcessTree(child.pid);
            });
    ...
    protected async collectArtifact(workspaceRoot: string, outputType: ArenaOutputType, fallbackRaw: string): Promise<ArenaArtifact> {
        ...
        return {
            ...
            error: files.length ? null : nls.localize('theia/arena/codexNoOutputFiles', 'Codex CLI did not write any files under output/.')
        };
    }
