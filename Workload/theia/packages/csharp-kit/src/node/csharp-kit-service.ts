...
interface LanguageDependencyHint {
    sourceSymbolId: string;
...
interface LanguageAnalysisContext {
...
export class CSharpKitServiceImpl implements CSharpKitService {
    ...
    protected createRoslynAnalyzer(analyzerPath: string | undefined, timeoutMs?: number): CSharpRoslynAnalyzerAdapter {
        return {
            analyze: context => ({
                analyzerId: 'csharp-kit-roslyn-unbundled',
                symbols: [],
                relations: [],
                diagnostics: [{
                    id: 'roslyn-sidecar-unbundled',
                    severity: 'info',
                    message: 'Roslyn semantic inventory is not bundled with the public C# Kit installer.',
                    path: context.file.relativePath,
                    detail: analyzerPath
                        ? `A Roslyn analyzer path was configured (${analyzerPath}), but the Memory Roslyn adapter is not installed in this CyberVinci build.`
                        : `Configure the standalone Roslyn adapter in a future C# semantic package to enable semantic inventory. Timeout: ${timeoutMs ?? this.roslynTimeout()}ms.`
                }]
            })
        };
    }
