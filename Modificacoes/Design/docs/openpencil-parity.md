...
- `OpenPencil: Generate with AI` uses a provider hook (`OpenPencilAiDesignProvider`) inside the Theia process. The primary provider routes through the provider-neutral CyberVinci AI Runtime so each Canvas AI request can choose provider, model, and reasoning effort independently. Canvas AI first asks the selected runtime to stream newline-delimited operation events, applies each validated operation to the canvas immediately, and falls back to skeleton/content/refinement stages when a provider cannot stream usable operation events. Legacy language-model/backend providers remain as fallback paths.
...
- Worktree: OpenPencil R2 documentation and contract pass in `C:\Users\<USER>\Desktop\theia-op-complete-r2-docs-20260517`
- Node: `v26.1.0`
- npm: `11.12.0`
