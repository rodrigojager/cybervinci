...
describe('flattenToRenderNodes — dimension consistency', () => {
  ...
  it('does not clip root frame children unless clipContent is enabled', () => {
    const root = frame({
      id: 'root',
      width: 400,
      height: 600,
    ...
    expect(t1.clipRect).toBeUndefined();
  });

  it('explicit root frame clipRect matches absW/absH, not a divergent nodeW/nodeH', () => {
    const root = frame({
      id: 'root',
      width: 400,
      height: 600,
  ...
  it('explicit root frame clipRect matches absW/absH for frame without explicit height', () => {
