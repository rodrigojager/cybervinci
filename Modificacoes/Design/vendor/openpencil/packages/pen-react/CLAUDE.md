...
- Peer support is `react`/`react-dom` `^18.3.1 || ^19.0.0`.
- The package relies on React 18-stable APIs, including `useSyncExternalStore`.
- Do not add React 19-only APIs (`use`, `useActionState`, `useOptimistic`) without also raising the peer dependency floor.
