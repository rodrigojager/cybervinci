...
**Peer dependencies:** `react@^18.3.1 || ^19.0.0`, `react-dom@^18.3.1 || ^19.0.0`, `@radix-ui/react-*` (popover, select, separator, slider, switch, toggle, tooltip)

## React Compatibility

`pen-react` is compatible with React 18.3.1 and React 19. It uses stable React APIs available in React 18, including `useSyncExternalStore`, and does not use React 19-only APIs such as `use`, `useActionState`, or `useOptimistic`.

Run the static compatibility check with:

```bash
npm run check:react18
```
