...
describe('AgentMarkdownStore', () => {
    ...
    let originalCatalogRoot: string | undefined;
    ...
    beforeEach(async () => {
        ...
        originalCatalogRoot = process.env.FLOW_AGENT_CATALOG_ROOT;
    });

    afterEach(async () => {
        if (originalCatalogRoot === undefined) {
            delete process.env.FLOW_AGENT_CATALOG_ROOT;
        } else {
            process.env.FLOW_AGENT_CATALOG_ROOT = originalCatalogRoot;
        }
    ...
    it('lists agency catalog agents and imports them as editable workspace Markdown', async () => {
        const catalogRoot = path.join(tempDir, 'catalog');
        await fs.mkdir(path.join(catalogRoot, 'engineering'), { recursive: true });
        await fs.writeFile(
            path.join(catalogRoot, 'engineering', 'frontend-developer.md'),
            '# Frontend Developer\n\nBuild focused UI changes.\n',
            'utf8'
        );
        process.env.FLOW_AGENT_CATALOG_ROOT = catalogRoot;

        const catalog = await store.listCatalogAgents(workspaceRootUri);
        const imported = await store.importCatalogAgent(workspaceRootUri, catalog[0].id);
        const relisted = await store.listCatalogAgents(workspaceRootUri);

        expect(catalog).to.have.length(1);
        expect(catalog[0].title).to.equal('Frontend Developer');
        expect(catalog[0].category).to.equal('engineering');
        expect(catalog[0].workspaceRelativePath).to.equal('agency/engineering/frontend-developer.md');
        expect(catalog[0].imported).to.equal(false);
        expect(imported.relativePath).to.equal('agency/engineering/frontend-developer.md');
        expect(imported.content).to.contain('Build focused UI changes.');
        expect(relisted[0].imported).to.equal(true);
    });
