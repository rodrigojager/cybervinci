...
import { FlowAgentCatalogSummary, FlowService } from '../common/flow-protocol';
...
describe('Flow browser integration', () => {
    ...
    it('lets chat-triggered saved-flow runs pick a saved workflow before starting', async () => {
        const widget = new TestFlowWidget();
        const workflows: FlowWorkflow[] = [
            workflowFixture(),
            {
                ...
                id: 'qa_review',
                name: 'QA Review',
                description: 'Review implementation details before final delivery.'
            }
        ];
        const started: unknown[] = [];
        widget.injectExternalRunForTest(
            {
                listWorkflows: async () => workflows,
                getWorkflow: async request => workflows.find(workflow => workflow.id === request.workflowId) || workflows[0],
                validateWorkflow: async () => ({ valid: true, errors: [], warnings: [] }),
            ...
            } as Pick<QuickInputService, 'showQuickPick'>
        );

        await widget.runWorkflowFromExternalPrompt({ prompt: 'Review this change.', selectWorkflow: true });
        ...
        expect(widget.activeRunForTest()?.workflowId).to.equal('qa_review');
        expect(widget.activeRunForTest()?.prompt).to.equal('Review this change.');
    });

    it('asks for AI-authored dynamic workflow clarification and reruns with the answer', async () => {
        const widget = new TestFlowWidget();
        const workflow = workflowFixture();
        const dynamicRequests: Array<{ prompt: string; authoringDraft?: unknown }> = [];
        ...
        widget.injectExternalRunForTest(
            {
                ...
                runDynamicWorkflow: async request => {
                    dynamicRequests.push({ prompt: request.prompt, authoringDraft: request.authoringDraft });
                    if (dynamicRequests.length === 1) {
                        throw new Error('Dynamic workflow needs user input: Which target environment should this workflow optimize for?');
                    }
                    return runFixture({
            ...
            },
            {
                input: async options => {
                    ...
                    return 'Node.js backend and browser UI.';
                }
            }
        );

        await widget.runDynamicWorkflowFromExternalPrompt({ prompt: 'Build an implementation workflow.' });

        expect(inputPrompts).to.deep.equal(['Which target environment should this workflow optimize for?']);
        expect(dynamicRequests).to.have.length(2);
        expect(dynamicRequests[1].prompt).to.contain('Build an implementation workflow.');
        expect(dynamicRequests[1].prompt).to.contain('Question: Which target environment should this workflow optimize for?');
        expect(dynamicRequests[1].prompt).to.contain('Answer: Node.js backend and browser UI.');
        expect(dynamicRequests[1].authoringDraft).to.equal(undefined);
        expect(widget.activeRunForTest()?.prompt).to.equal(dynamicRequests[1].prompt);
    });

    it('opens manually inserted agent states through a generated Markdown path and workflow mapping', async () => {
        const widget = new TestFlowWidget();
        const workflow = workflowFixture();
        const agentRequests: unknown[] = [];
        const openedUris: string[] = [];

        widget.injectAgentOpenForTest(
            {
                validateWorkflow: async () => ({ valid: true, errors: [], warnings: [] }),
                getAgentMarkdownFile: async request => {
                    agentRequests.push(request);
                    return {
                        path: 'C:/workspace/project/.theia/flow/agents/agents/frontend.md',
                        uri: 'file:///workspace/project/.theia/flow/agents/agents/frontend.md',
        ...
        await widget.openAgentMarkdownForTest('frontend');

        expect(agentRequests).to.deep.equal([{
            workspaceRootUri: 'file:///workspace/project',
            relativePath: 'agents/frontend.md',
            title: 'frontend',
            createIfMissing: true
        }]);
        expect(widget.activeWorkflowForTest()?.agents).to.deep.equal({
            frontend: 'agents/frontend.md'
        });
        expect(openedUris).to.deep.equal(['file:///workspace/project/.theia/flow/agents/agents/frontend.md']);
    });

    it('adds a catalog agent as a workflow node with a workspace Markdown mapping', async () => {
        const widget = new TestFlowWidget();
        const workflow = workflowFixture();
        const catalogAgent = catalogAgentFixture();
        const importRequests: unknown[] = [];

        widget.injectAgentCatalogForTest(
            {
                validateWorkflow: async () => ({ valid: true, errors: [], warnings: [] }),
                importAgentCatalogFile: async request => {
                    importRequests.push(request);
                    return {
                        path: 'C:/workspace/project/.theia/flow/agents/agency/engineering/engineering-frontend-developer.md',
                        uri: 'file:///workspace/project/.theia/flow/agents/agency/engineering/engineering-frontend-developer.md',
                        relativePath: catalogAgent.workspaceRelativePath,
                        content: '# Frontend Developer\n',
                        updatedAt: '2026-06-06T00:00:00.000Z'
                    };
                },
                listAgentMarkdownFiles: async () => [{
                    path: 'C:/workspace/project/.theia/flow/agents/agency/engineering/engineering-frontend-developer.md',
                    uri: 'file:///workspace/project/.theia/flow/agents/agency/engineering/engineering-frontend-developer.md',
                    relativePath: catalogAgent.workspaceRelativePath,
                    updatedAt: '2026-06-06T00:00:00.000Z'
                }],
                listAgentCatalog: async () => [{ ...catalogAgent, imported: true }]
            },
            workflow,
            [catalogAgent]
        );

        const stateId = await widget.addAgentNodeFromCatalogForTest(catalogAgent.id);
        const activeWorkflow = widget.activeWorkflowForTest();

        expect(importRequests).to.deep.equal([{
            workspaceRootUri: 'file:///workspace/project',
            catalogId: catalogAgent.id
        }]);
        expect(stateId).to.equal('agent');
        expect(activeWorkflow?.agents?.engineering_engineering_frontend_developer).to.equal(catalogAgent.workspaceRelativePath);
        expect(activeWorkflow?.states.agent.agent).to.equal('engineering_engineering_frontend_developer');
    });
});
...
class TestFlowWidget extends FlowWidget {
    ...
    injectExternalRunForTest(
        service: Pick<FlowService, 'listWorkflows' | 'getWorkflow' | 'validateWorkflow'> & Partial<Pick<FlowService, 'startRun' | 'runDynamicWorkflow'>>,
        quickInputService: Partial<Pick<QuickInputService, 'showQuickPick' | 'input'>>
    ): void {
        const workflow = workflowFixture();
        Object.defineProperty(this, 'flowService', { value: service });
        Object.defineProperty(this, 'quickInputService', { value: quickInputService });
        Object.defineProperty(this, 'workspaceService', {
            value: {
                roots: Promise.resolve([{ resource: { toString: () => 'file:///workspace/project' } }])
            }
        });
        this.state = {
            ...this.state,
    ...
    injectAgentOpenForTest(
        service: Pick<FlowService, 'validateWorkflow' | 'getAgentMarkdownFile'>,
        openerService: { getOpener: (uri: unknown, options?: unknown) => Promise<{ open: (uri: { toString(): string }, options?: unknown) => Promise<object> }> },
        workflow: FlowWorkflow
    ): void {
        Object.defineProperty(this, 'flowService', { value: service });
        Object.defineProperty(this, 'openerService', { value: openerService });
        Object.defineProperty(this, 'workspaceService', {
            value: {
                roots: Promise.resolve([{ resource: { toString: () => 'file:///workspace/project' } }])
            }
    ...
    injectAgentCatalogForTest(
        service: Pick<FlowService, 'validateWorkflow' | 'importAgentCatalogFile' | 'listAgentMarkdownFiles' | 'listAgentCatalog'>,
        workflow: FlowWorkflow,
    ...
    protected override async subscribeActiveRunStream(_runId: string | undefined): Promise<void> {
        return undefined;
    }
    ...
    activeWorkflowForTest(): FlowWorkflow | undefined {
        return this.state.snapshot?.activeWorkflow;
    }

    openAgentMarkdownForTest(agentIdOrPath: string): Promise<void> {
        return this.openAgentMarkdown(agentIdOrPath);
    }

    addAgentNodeFromCatalogForTest(catalogId: string): Promise<string | undefined> {
        return this.addAgentNodeFromCatalog(catalogId);
    }
...
function catalogAgentFixture(): FlowAgentCatalogSummary {
    return {
        id: 'agency:engineering/engineering-frontend-developer.md',
        title: 'Frontend Developer Agent Personality',
        category: 'engineering',
        sourceRelativePath: 'engineering/engineering-frontend-developer.md',
        sourcePath: 'C:/catalog/engineering/engineering-frontend-developer.md',
        sourceUri: 'file:///catalog/engineering/engineering-frontend-developer.md',
        workspaceRelativePath: 'agency/engineering/engineering-frontend-developer.md',
        imported: false,
        updatedAt: '2026-06-06T00:00:00.000Z',
        excerpt: 'Build focused UI changes.'
    };
}
...
function runFixture(partial: Partial<FlowRun>): FlowRun {
    return {
        id: 'run-1',
        workflowId: partial.workflowId || 'contracted_delivery',
        prompt: partial.prompt || 'ship feature',
