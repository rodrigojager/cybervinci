...
import { QuickInputService, QuickPickItem } from '@theia/core';
...
const flowMarkdownRenderer = markdownit({
    html: false,
    linkify: true,
    breaks: false
...
interface FlowWidgetState {
    ...
    agentCatalog: FlowAgentCatalogSummary[];
    agentSearch: string;
    agentCatalogCategory: string;
    agentCatalogVisible: boolean;
    agentCatalogTargetStateId?: string;
    agentCatalogPreview?: FlowAgentCatalogFile;
    agentCatalogPreviewLoading: boolean;
    agentCatalogPreviewError?: string;
...
type FlowTopMenu = 'workflow' | 'file' | 'workloads' | 'run' | 'events' | 'history';
...
const DYNAMIC_WORKFLOW_INPUT_PREFIX = 'Dynamic workflow needs user input: ';
const MAX_DYNAMIC_WORKFLOW_CLARIFICATIONS = 2;
...
export class FlowWidget extends ReactWidget {
    ...
    protected state: FlowWidgetState = {
        ...
        agentCatalogVisible: false,
        agentCatalogPreviewLoading: false,
        patternParameters: {},
    ...
    readonly refresh = async (): Promise<void> => {
        await this.withBusy(async () => {
            ...
            const [snapshot, templates, workflowPatterns, modelProfiles, pipelinePresets, agents, agentCatalog, languageModels] = await Promise.all([
                ...
                this.flowService.listAgentCatalog({ workspaceRootUri }),
        ...
        return <div className='flow'>
            <header className='flow__header'>
                ...
                <nav className='flow__top-menus' aria-label='Flow menus'>
                    ...
                    <button
                        className={this.state.agentCatalogVisible ? 'flow__history-button flow__history-button--open' : 'flow__history-button'}
                        title='Abrir catalogo de agentes'
                        onClick={() => this.openAgentCatalog()}
                        disabled={this.state.busy}
                    >
                        <i className='codicon codicon-hubot' /> Agents
                    </button>
                    <div className={`flow__menu flow__menu--workloads ${this.state.openMenu === 'workloads' ? 'flow__menu--open' : ''}`}>
                        <button
                            type='button'
                            className='flow__menu-trigger'
                            aria-haspopup='menu'
                            aria-expanded={this.state.openMenu === 'workloads'}
                            onClick={() => this.toggleTopMenu('workloads')}
                        >
                            <i className='codicon codicon-list-tree' /> Workloads
                        </button>
                        {this.state.openMenu === 'workloads' && <div className='flow__menu-panel flow__menu-panel--ops flow__menu-panel--workloads'>
                            <Kanban run={run} onOpenArtifact={this.openArtifact} />
                        </div>}
                    </div>
                    <div className={`flow__menu flow__menu--run ${this.state.openMenu === 'run' ? 'flow__menu--open' : ''}`}>
                        <button
                            type='button'
                            className='flow__menu-trigger'
                            aria-haspopup='menu'
                            aria-expanded={this.state.openMenu === 'run'}
                            onClick={() => this.toggleTopMenu('run')}
                        >
                            <i className='codicon codicon-dashboard' /> Run
                        </button>
                        {this.state.openMenu === 'run' && <div className='flow__menu-panel flow__menu-panel--ops flow__menu-panel--run'>
                            <RunObservability
                                run={run}
                    ...
                    <div className={`flow__menu flow__menu--events ${this.state.openMenu === 'events' ? 'flow__menu--open' : ''}`}>
                        <button
                            type='button'
                            className='flow__menu-trigger'
                            aria-haspopup='menu'
                            aria-expanded={this.state.openMenu === 'events'}
                            onClick={() => this.toggleTopMenu('events')}
                        >
                            <i className='codicon codicon-list-filter' /> Eventos
                        </button>
                        {this.state.openMenu === 'events' && <div className='flow__menu-panel flow__menu-panel--ops flow__menu-panel--events'>
            ...
            {this.state.agentCatalogVisible && <AgentCatalogModal
                catalog={this.state.agentCatalog}
                search={this.state.agentSearch}
                category={this.state.agentCatalogCategory}
                preview={this.state.agentCatalogPreview}
                previewLoading={this.state.agentCatalogPreviewLoading}
                previewError={this.state.agentCatalogPreviewError}
                busy={this.state.busy}
                targetStateId={this.state.agentCatalogTargetStateId}
                onSearch={this.setAgentSearch}
                onCategory={this.setAgentCatalogCategory}
                onPreview={this.previewAgentCatalog}
                onUse={this.useCatalogAgent}
                onOpenMarkdown={this.openCatalogAgentMarkdown}
                onCreate={this.createAgentMarkdown}
                onClose={this.closeAgentCatalog}
            />}
            ...
            <main className='flow__main'>
                ...
                <aside className='flow__inspector'>
                    <Inspector
                        ...
                        onPickAgent={this.pickAgentForState}
                        onOpenAgent={this.openAgentMarkdown}
                        agents={this.state.agents}
                        agentCatalog={this.state.agentCatalog}
    ...
    protected readonly handleTopMenuKeyDown = (event: KeyboardEvent): void => {
        if (event.key === 'Escape') {
            if (this.state.agentCatalogVisible) {
                this.closeAgentCatalog();
                return;
            }
            if (this.state.openMenu || this.state.runHistoryVisible) {
                this.closeTopMenus();
    ...
    protected readonly setAgentCatalogCategory = (agentCatalogCategory: string): void => {
        this.state = { ...this.state, agentCatalogCategory };
        this.update();
    };
    ...
    protected readonly createAgentMarkdown = async (): Promise<void> => {
        ...
        await this.withBusy(async () => {
            ...
            const [agents, agentCatalog] = await Promise.all([
                this.flowService.listAgentMarkdownFiles({ workspaceRootUri }),
                this.flowService.listAgentCatalog({ workspaceRootUri })
            ]);
            this.state = { ...this.state, agents, agentCatalog, agentSearch: relativePath, error: undefined };
    ...
    protected readonly duplicateAgentMarkdown = async (sourceRelativePath: string): Promise<void> => {
        ...
        await this.withBusy(async () => {
            ...
            this.state = { ...this.state, agents, agentCatalog, agentSearch: targetRelativePath, error: undefined };
            await open(this.openerService, new URI(agent.uri));
        });
    };
    ...
    protected readonly addAgentNodeFromCatalog = async (catalogId?: string): Promise<string | undefined> => {
        const snapshot = this.state.snapshot;
        const workflow = snapshot?.activeWorkflow;
        if (!snapshot || !workflow) {
            return undefined;
        }
        const agent = catalogId
            ? this.state.agentCatalog.find(candidate => candidate.id === catalogId)
            : await this.pickAgentCatalogItem();
        if (!agent) {
            return undefined;
        }
        const imported = await this.importCatalogAgent(agent);
        const { workflow: nextWorkflow, stateId } = addFlowWorkflowState(workflow, 'agent');
        const assigned = assignAgentToWorkflowState(nextWorkflow, stateId, imported.relativePath, agent);
        await this.replaceActiveWorkflow(assigned.workflow, 'state', stateId);
        return stateId;
    };

    protected readonly pickAgentForState = async (stateId: string): Promise<void> => {
        this.openAgentCatalog(stateId);
    };

    protected openAgentCatalog(targetStateId?: string): void {
        this.state = {
            ...this.state,
            openMenu: undefined,
            runHistoryVisible: false,
            agentCatalogVisible: true,
            agentCatalogTargetStateId: targetStateId,
            agentCatalogPreviewError: undefined
        };
        this.update();
        const previewId = this.state.agentCatalogPreview?.id || this.state.agentCatalog[0]?.id;
        if (previewId) {
            void this.previewAgentCatalog(previewId);
        }
    }

    protected readonly closeAgentCatalog = (): void => {
        if (!this.state.agentCatalogVisible) {
            return;
        }
        this.state = {
            ...this.state,
            agentCatalogVisible: false,
            agentCatalogTargetStateId: undefined,
            agentCatalogPreviewLoading: false,
            agentCatalogPreviewError: undefined
        };
        this.update();
    ...
    protected readonly previewAgentCatalog = async (catalogId: string): Promise<void> => {
        this.state = {
            ...this.state,
            agentCatalogPreviewLoading: true,
            agentCatalogPreviewError: undefined
        };
        this.update();
        try {
            const workspaceRootUri = await this.workspaceRootUri();
            const preview = await this.flowService.getAgentCatalogFile({ workspaceRootUri, catalogId });
            this.state = {
                ...this.state,
                agentCatalogPreview: preview,
                agentCatalogPreviewLoading: false,
                agentCatalogPreviewError: undefined
            };
            this.update();
            ...
            this.state = {
                ...this.state,
                agentCatalogPreviewLoading: false,
                agentCatalogPreviewError: error instanceof Error ? error.message : String(error)
            };
            this.update();
        }
    };

    protected readonly useCatalogAgent = async (catalogId: string): Promise<void> => {
        const targetStateId = this.state.agentCatalogTargetStateId;
        if (targetStateId) {
            await this.assignCatalogAgentToState(targetStateId, catalogId);
        } else {
            await this.addAgentNodeFromCatalog(catalogId);
        }
        this.closeAgentCatalog();
    };

    protected async assignCatalogAgentToState(stateId: string, catalogId: string): Promise<void> {
        const snapshot = this.state.snapshot;
        const workflow = snapshot?.activeWorkflow;
        if (!snapshot || !workflow) {
            return;
        }
        const agent = this.state.agentCatalog.find(candidate => candidate.id === catalogId);
        if (!agent) {
            return;
        }
        const imported = await this.importCatalogAgent(agent);
        const assigned = assignAgentToWorkflowState(workflow, stateId, imported.relativePath, agent);
        await this.replaceActiveWorkflow(assigned.workflow, 'state', stateId);
    }

    protected async pickAgentCatalogItem(): Promise<FlowAgentCatalogSummary | undefined> {
        const catalog = this.state.agentCatalog;
        if (catalog.length === 0) {
            this.state = { ...this.state, error: 'Agent catalog is empty. Configure FLOW_AGENT_CATALOG_ROOT or add Markdown agents to the Agency Agents library.' };
            this.update();
            return undefined;
        }
        interface AgentCatalogPick extends QuickPickItem {
            agent: FlowAgentCatalogSummary;
        }
        const selected = await this.quickInputService.showQuickPick<AgentCatalogPick>(
            catalog.map(agent => ({
                label: agent.title,
                description: `${agent.category}${agent.imported ? ' / importado' : ''}`,
                detail: `${agent.sourceRelativePath}${agent.excerpt ? ` - ${agent.excerpt}` : ''}`,
                agent
            })),
            {
                placeholder: 'Escolha um agente do catalogo'
            }
        );
        return selected?.agent;
    }

    protected async importCatalogAgent(agent: FlowAgentCatalogSummary): Promise<FlowAgentMarkdownSummary> {
        const workspaceRootUri = await this.workspaceRootUri();
        const imported = await this.flowService.importAgentCatalogFile({
            workspaceRootUri,
            catalogId: agent.id
        });
        const [agents, agentCatalog] = await Promise.all([
    ...
    protected readonly openCatalogAgentMarkdown = async (catalogId: string): Promise<void> => {
        const agent = this.state.agentCatalog.find(candidate => candidate.id === catalogId);
        if (!agent) {
            return;
        }
        const imported = await this.importCatalogAgent(agent);
        await this.openAgentMarkdown(imported.relativePath);
    };
    ...
    async runWorkflowFromExternalPrompt(options: FlowExternalRunOptions = {}): Promise<void> {
        await this.withBusy(async () => {
            ...
            const workflow = await this.resolveExternalRunWorkflow(workspaceRootUri, options);
            if (!workflow) {
    ...
    protected async resolveExternalRunWorkflow(workspaceRootUri: string | undefined, options: FlowExternalRunOptions): Promise<FlowWorkflow | undefined> {
        if (options.workflowId) {
            return this.flowService.getWorkflow({ workspaceRootUri, workflowId: options.workflowId });
        }
        if (options.selectWorkflow) {
            const workflows = await this.flowService.listWorkflows({ workspaceRootUri });
            return this.pickWorkflowForExternalRun(workflows, this.state.snapshot?.activeWorkflow?.id);
        }
        return this.state.snapshot?.activeWorkflow;
    }

    protected async pickWorkflowForExternalRun(workflows: FlowWorkflow[], activeWorkflowId: string | undefined): Promise<FlowWorkflow | undefined> {
        ...
        interface FlowWorkflowPick extends QuickPickItem {
            workflow: FlowWorkflow;
        }
        const selected = await this.quickInputService.showQuickPick<FlowWorkflowPick>(
            workflows.map(workflow => ({
                label: workflow.name || workflow.id,
                description: workflow.id === activeWorkflowId
                    ? nls.localize('theia/flow/workflowPicker/activeDescription', 'active')
                    : workflow.id,
                detail: workflow.description,
                workflow
            })),
            {
                placeholder: nls.localize('theia/flow/workflowPicker/placeholder', 'Choose a saved workflow')
            }
        );
    ...
    async runDynamicWorkflowFromExternalPrompt(options: FlowExternalRunOptions = {}): Promise<void> {
        await this.withBusy(async () => {
            const workspaceRootUri = options.workspaceRootUri || await this.workspaceRootUri();
            const prompt = externalPromptText(options) || this.state.prompt;
            const result = await this.runDynamicWorkflowWithClarification({
                workspaceRootUri,
                prompt,
            ...
            await this.applyExternalRunState(workspaceRootUri, result.run.workflowId, result.run, result.prompt);
        }, error => classifyExecutionModeFromError(error));
    }
    ...
    protected async runDynamicWorkflowWithClarification(request: FlowRunDynamicWorkflowRequest): Promise<{ run: FlowRun; prompt: string }> {
        let prompt = request.prompt;
        let nextRequest: FlowRunDynamicWorkflowRequest = request;
        let lastQuestionError: unknown;
        for (let attempt = 0; attempt <= MAX_DYNAMIC_WORKFLOW_CLARIFICATIONS; attempt++) {
            try {
                return {
                    run: await this.flowService.runDynamicWorkflow(nextRequest),
                    prompt
                };
            } catch (error) {
                const question = dynamicWorkflowQuestionFromError(error);
                if (!question || attempt >= MAX_DYNAMIC_WORKFLOW_CLARIFICATIONS) {
                    throw error;
                }
                lastQuestionError = error;
                const answer = await this.quickInputService.input({
                    title: nls.localize('theia/flow/dynamicWorkflow/clarificationTitle', 'Dynamic Flow'),
                    prompt: question,
                    placeHolder: nls.localize('theia/flow/dynamicWorkflow/clarificationPlaceholder', 'Type the missing detail and press Enter'),
                    ignoreFocusLost: true,
                    validateInput: async value => value.trim()
                        ? undefined
                        : nls.localize('theia/flow/dynamicWorkflow/clarificationRequired', 'Enter a clarification to continue the dynamic workflow.')
                });
                if (!answer?.trim()) {
                    throw lastQuestionError;
                }
                prompt = appendDynamicWorkflowClarification(prompt, question, answer.trim());
                nextRequest = {
                    ...request,
                    prompt,
        ...
        throw lastQuestionError instanceof Error ? lastQuestionError : new Error(String(lastQuestionError));
    }
    ...
    protected readonly openAgentMarkdown = async (agentIdOrPath: string): Promise<void> => {
        ...
        await this.withBusy(async () => {
            const resolved = resolveAgentMarkdownReference(workflow, agentIdOrPath);
            const agentId = resolved.agentId;
            const shouldMapAgent = agentId
                && workflow.file?.editable !== false
                && workflow.agents?.[agentId] !== resolved.relativePath;
            if (shouldMapAgent) {
                await this.setActiveWorkflow({
                    ...workflow,
                    agents: {
                        ...(workflow.agents || {}),
                        [agentId]: resolved.relativePath
                    }
                }, this.state.selectedKind, this.state.selectedId, true);
            }
...
function AgentCatalogModal(props: {
    catalog: FlowAgentCatalogSummary[];
    search: string;
    category: string;
    preview?: FlowAgentCatalogFile;
    ...
    onPreview: (catalogId: string) => Promise<void>;
    onUse: (catalogId: string) => Promise<void>;
    onOpenMarkdown: (catalogId: string) => Promise<void>;
    onCreate: () => Promise<void>;
    onClose: () => void;
}): React.ReactElement {
    const query = props.search.trim().toLowerCase();
    const categories = Array.from(new Set(props.catalog.map(agent => agent.category))).sort();
    const agents = props.catalog.filter(agent => {
        if (props.category && agent.category !== props.category) {
            return false;
        }
        if (!query) {
            return true;
        ...
        return [
            ...
            agent.workspaceRelativePath,
            agent.excerpt
        ].filter(Boolean).join(' ').toLowerCase().includes(query);
    });
    const selectedId = props.preview?.id;
    const actionLabel = props.targetStateId ? 'Usar neste nó' : 'Adicionar como nó';
    const actionTitle = props.targetStateId
        ? 'Usar este agente no nó selecionado'
        : 'Adicionar este agente como novo nó do workflow';
    const closeOnBackdrop = (event: React.MouseEvent<HTMLDivElement>): void => {
        if (event.target === event.currentTarget) {
            props.onClose();
        }
    };
    return <div className='flow__modal-backdrop' role='presentation' onMouseDown={closeOnBackdrop}>
        <section className='flow__agent-catalog-modal' role='dialog' aria-modal='true' aria-label='Agent catalog'>
            <header className='flow__agent-catalog-header'>
                <div>
                    <h3>Agent Catalog</h3>
                    <span>{agents.length} de {props.catalog.length}</span>
                </div>
                <div>
                    <button title='Create agent Markdown' onClick={props.onCreate} disabled={props.busy}>
                        <i className='codicon codicon-add' /> Novo
                    </button>
                    <button title='Fechar catalogo' onClick={props.onClose}>
                        <i className='codicon codicon-close' />
                    </button>
                </div>
            </header>
            <div className='flow__agent-catalog-body'>
                <aside className='flow__agent-catalog-sidebar'>
                    <label className='flow__agent-library-category'>
                        <span>Categoria</span>
                        <select
                            value={props.category}
                            onChange={event => props.onCategory(event.currentTarget.value)}
                            disabled={props.busy}
                            aria-label='Filtrar categoria de agente'
                        >
                    ...
                    <div className='flow__agent-library-search'>
                        ...
                        <input
                            ...
                            aria-label='Buscar agentes do catalogo'
                        />
                    </div>
                    <div className='flow__agent-library-list'>
                        {agents.length === 0 && <p>Nenhum agente encontrado.</p>}
                        {agents.map(agent => <button
                            key={agent.id}
                            type='button'
                            className={agent.id === selectedId ? 'flow__agent-catalog-item flow__agent-catalog-item--selected' : 'flow__agent-catalog-item'}
                            onClick={() => props.onPreview(agent.id)}
                            disabled={props.busy}
                        >
                            <i className={agent.imported ? 'codicon codicon-check' : 'codicon codicon-symbol-class'} />
                            <span>{agent.title}</span>
                            <small>{agent.category} / {agent.sourceRelativePath}</small>
                        </button>)}
                    </div>
                </aside>
                <main className='flow__agent-catalog-preview'>
                    {!props.preview && !props.previewLoading && !props.previewError && <p>Selecione um agente para visualizar o Markdown renderizado.</p>}
                    {props.previewLoading && <p>Carregando agente...</p>}
                    {props.previewError && <div className='flow__inline-validation'><span>{props.previewError}</span></div>}
                    {props.preview && <>
                        <div className='flow__agent-catalog-preview-header'>
                            <div>
                                <h4>{props.preview.title}</h4>
                                <span>{props.preview.category} / {props.preview.sourceRelativePath}</span>
                            </div>
                            <div>
                                <button title={actionTitle} onClick={() => props.onUse(props.preview!.id)} disabled={props.busy}>
                                    <i className='codicon codicon-add' /> {actionLabel}
                                </button>
                                <button onClick={() => props.onOpenMarkdown(props.preview!.id)} disabled={props.busy}>
                                    <i className='codicon codicon-edit' /> Editar Markdown
                                </button>
                            </div>
                        </div>
                        <MarkdownReportViewer report={props.preview.content} />
                    </>}
                </main>
            </div>
        </section>
...
function Inspector(props: {
    ...
    onPickAgent: (stateId: string) => Promise<void>;
...
function StateEditor(props: {
    ...
    const agentOptions = knownAgentOptions(props.workflow, props.agents, props.state.agent);
    const selectedCatalogAgent = catalogAgentForState(props.workflow, props.agentCatalog, props.state.agent);
        ...
        <div className='flow__form-section'>
            ...
            <label>
                ...
                <div className='flow__agent-field'>
                    <select
                        value={agentOptions.some(option => option.value === props.state.agent) ? props.state.agent || '' : ''}
                        disabled={!props.editable || agentOptions.length === 0}
                        title='Select known agent Markdown'
                        aria-label='Known agent Markdown'
                        onChange={event => props.onUpdateState(props.stateId, { agent: emptyToUndefined(event.currentTarget.value) })}
                    >
                        <option value=''>Known agents...</option>
                    ...
                    />
                    <button
                        disabled={!props.editable || props.agentCatalog.length === 0}
                        title='Choose an agent from the catalog'
                        onClick={() => props.onPickAgent(props.stateId)}
                    >
            ...
            {selectedCatalogAgent && <div className='flow__agent-selection-summary'>
                <strong>{selectedCatalogAgent.title}</strong>
                <span>{selectedCatalogAgent.category} / {selectedCatalogAgent.sourceRelativePath}</span>
            </div>}
...
function AgentMarkdownActions(props: {
    ...
    const relativePath = resolveAgentMarkdownReference(props.workflow, props.agentIdOrPath).relativePath;
...
function MarkdownReportViewer(props: { report: string }): React.ReactElement {
    const markdown = redactFlowSecretsText(props.report) || 'No report content available in the workload envelope.';
    const parsed = splitMarkdownFrontmatter(markdown);
    const html = DOMPurify.sanitize(flowMarkdownRenderer.render(parsed.body || 'No report content available in the workload envelope.'));
    return <div className='flow__artifact-markdown'>
        {parsed.frontmatter && <MarkdownFrontmatterView frontmatter={parsed.frontmatter} />}
        <div className='flow__artifact-markdown-body' dangerouslySetInnerHTML={{ __html: html }} />
    </div>;
}

function MarkdownFrontmatterView(props: { frontmatter: Record<string, unknown> }): React.ReactElement {
    const entries = orderedFrontmatterEntries(props.frontmatter);
    return <dl className='flow__markdown-frontmatter' aria-label='Markdown metadata'>
        {entries.map(([key, value]) => <div key={key}>
            <dt>{key}</dt>
            <dd>
                {isColorValue(value) && <span className='flow__markdown-frontmatter-swatch' style={{ background: String(value) }} />}
...
function splitMarkdownFrontmatter(markdown: string): { frontmatter?: Record<string, unknown>; body: string } {
    ...
    try {
        ...
        if (!isObjectRecord(loaded)) {
            return { body: markdown.slice(match[0].length).trimStart() || markdown };
        }
        return {
            frontmatter: loaded,
...
function orderedFrontmatterEntries(frontmatter: Record<string, unknown>): Array<[string, unknown]> {
    const preferred = ['name', 'description', 'color', 'emoji', 'vibe'];
    const entries = Object.entries(frontmatter).filter(([, value]) => value !== undefined && value !== null && String(value).trim() !== '');
...
function isColorValue(value: unknown): boolean {
    ...
    return /^#[0-9a-f]{3,8}$/i.test(text) || /^(?:black|blue|cyan|gray|green|magenta|orange|pink|purple|red|white|yellow)$/i.test(text);
}
...
function resolveAgentMarkdownReference(workflow: FlowWorkflow, agentIdOrPath: string): { agentId?: string; relativePath: string } {
    const trimmed = agentIdOrPath.trim();
    const mapped = workflow.agents?.[trimmed];
    if (mapped) {
        return { agentId: trimmed, relativePath: normalizeAgentMarkdownPath(mapped) };
    }
    if (isMarkdownAgentPath(trimmed)) {
        return { relativePath: normalizeAgentMarkdownPath(trimmed) };
    }
    return {
        agentId: trimmed,
        relativePath: defaultAgentMarkdownPath(trimmed)
    };
}

function assignAgentToWorkflowState(
    workflow: FlowWorkflow,
    stateId: string,
    relativePath: string,
    catalogAgent: FlowAgentCatalogSummary
): { workflow: FlowWorkflow; agentId: string } {
    const existingState = findFlowWorkflowState(workflow, stateId);
    if (!existingState) {
        return { workflow, agentId: catalogAgentId(catalogAgent, relativePath) };
    }
    const normalizedPath = normalizeAgentMarkdownPath(relativePath);
    const agentId = workflowAgentIdForPath(workflow, normalizedPath, catalogAgent);
    ...
    return {
        agentId,
        workflow: replaceFlowWorkflowState(workflowWithAgent, stateId, updatedState)
    };
}

function workflowAgentIdForPath(workflow: FlowWorkflow, relativePath: string, catalogAgent: FlowAgentCatalogSummary): string {
    for (const [agentId, mappedPath] of Object.entries(workflow.agents || {})) {
        if (normalizeAgentMarkdownPath(mappedPath) === relativePath) {
            return agentId;
        }
    }
    const baseId = catalogAgentId(catalogAgent, relativePath);
    if (!workflow.agents?.[baseId]) {
        return baseId;
    }
...
function catalogAgentId(catalogAgent: FlowAgentCatalogSummary, relativePath: string): string {
    const source = catalogAgent.sourceRelativePath || relativePath.replace(/^agency\//, '');
    const withoutExtension = source.replace(/\.(md|markdown)$/i, '');
    const agentId = withoutExtension
        .split(/[\\/]+/)
        .map(segment => sanitizeAgentMarkdownSegment(segment).replace(/-/g, '_'))
        .filter(Boolean)
        .join('_');
    return /^[a-zA-Z_]/.test(agentId) ? agentId : `agent_${agentId || 'catalog'}`;
}

function catalogAgentForState(
    workflow: FlowWorkflow,
    catalog: FlowAgentCatalogSummary[],
    currentAgent?: string
): FlowAgentCatalogSummary | undefined {
    if (!currentAgent) {
        return undefined;
    }
    const relativePath = resolveAgentMarkdownReference(workflow, currentAgent).relativePath;
    return catalog.find(agent => normalizeAgentMarkdownPath(agent.workspaceRelativePath) === normalizeAgentMarkdownPath(relativePath));
}

function knownAgentOptions(
    workflow: FlowWorkflow,
    agents: FlowAgentMarkdownSummary[],
    currentAgent?: string
): Array<{ value: string; label: string }> {
    const options = new Map<string, string>();
    const knownMarkdownPaths = new Set<string>();
    for (const [agentId, relativePath] of Object.entries(workflow.agents || {})) {
        const normalized = normalizeAgentMarkdownPath(relativePath);
        options.set(agentId, `${agentId} - ${normalized}`);
        knownMarkdownPaths.add(normalized);
    }
    for (const agent of agents) {
        const relativePath = normalizeAgentMarkdownPath(agent.relativePath);
        if (!knownMarkdownPaths.has(relativePath)) {
            options.set(relativePath, relativePath);
            knownMarkdownPaths.add(relativePath);
        }
    }
    if (currentAgent && !options.has(currentAgent)) {
        const resolved = resolveAgentMarkdownReference(workflow, currentAgent);
        options.set(currentAgent, `${currentAgent} - ${resolved.relativePath}`);
    }
...
function defaultAgentMarkdownPath(agentIdOrPath: string): string {
    const normalized = normalizeAgentMarkdownPath(agentIdOrPath).replace(/\/+$/g, '');
    const base = normalized.includes('/') ? normalized : `agents/${sanitizeAgentMarkdownSegment(normalized)}`;
    return `${base || 'agents/agent'}.md`;
}

function normalizeAgentMarkdownPath(value: string): string {
    return value.trim().replace(/\\/g, '/').replace(/^\/+/, '');
}

function isMarkdownAgentPath(value: string): boolean {
    const normalized = normalizeAgentMarkdownPath(value).toLowerCase();
    return normalized.endsWith('.md') || normalized.endsWith('.markdown');
}

function sanitizeAgentMarkdownSegment(value: string): string {
    return value.trim().replace(/[^a-zA-Z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').toLowerCase() || 'agent';
}
...
function dynamicWorkflowQuestionFromError(error: unknown): string | undefined {
    const message = error instanceof Error ? error.message : String(error);
    if (!message.startsWith(DYNAMIC_WORKFLOW_INPUT_PREFIX)) {
        return undefined;
    }
    const question = message.slice(DYNAMIC_WORKFLOW_INPUT_PREFIX.length).trim();
    return question || undefined;
}

function appendDynamicWorkflowClarification(prompt: string, question: string, answer: string): string {
    return [
        prompt.trim(),
        '',
        'Dynamic workflow clarification:',
