...
import { FlowRunDynamicWorkflowRequest, FlowService, FlowWorkflow } from '@cybervinci/flow/lib/common';
...
export class CodexHostBackendServiceImpl implements CodexHostBackendService {
    ...
    protected async flowListWorkflows(params: unknown): Promise<Record<string, unknown>> {
        ...
        const workflows: FlowWorkflow[] = await flow.listWorkflows(workspaceRootUri ? { workspaceRootUri } : {});
        return { workflows };
    }
    ...
    protected async flowStartWorkflow(params: unknown): Promise<Record<string, unknown>> {
        ...
        const workspaceRootUri = this.extractFlowWorkspaceRootUri(record);
        const workflowId = await this.resolveFlowStartWorkflowId(flow, record, workspaceRootUri);
        const request = {
            workflowId,
            prompt
        };
    ...
    protected async resolveFlowStartWorkflowId(flow: FlowService, record: Record<string, unknown>, workspaceRootUri: string | undefined): Promise<string> {
        const explicitWorkflowId = this.extractString(record, ['workflowId', 'id']);
        if (explicitWorkflowId) {
            return explicitWorkflowId;
        }
        const workflowSelector = this.extractString(record, ['workflowName', 'name', 'workflow', 'savedWorkflow', 'savedWorkflowName', 'savedWorkflowId']);
        if (!workflowSelector) {
            throw new Error('flow-start-workflow requires workflowId or workflowName.');
        }
        const workflows = await flow.listWorkflows(workspaceRootUri ? { workspaceRootUri } : {});
        const exact = workflows.find(workflow => workflow.id === workflowSelector || workflow.name === workflowSelector);
        if (exact) {
            return exact.id;
        }
        const normalizedSelector = normalizeFlowWorkflowSelector(workflowSelector);
        const matches = workflows.filter(workflow =>
            normalizeFlowWorkflowSelector(workflow.id) === normalizedSelector
            || normalizeFlowWorkflowSelector(workflow.name) === normalizedSelector
        );
        if (matches.length === 1) {
            return matches[0].id;
        }
        if (matches.length > 1) {
            throw new Error(`flow-start-workflow workflow selector "${workflowSelector}" is ambiguous. Matching workflows: ${matches.map(workflow => `"${workflow.id}"`).join(', ')}.`);
        }
        throw new Error(`flow-start-workflow could not resolve workflow "${workflowSelector}". Use flow-list-workflows and pass workflowId or workflowName.`);
    }
    ...
    protected async flowRunDynamicWorkflow(params: unknown): Promise<Record<string, unknown>> {
        ...
        const roleOverrides = this.isRecord(record.roleOverrides)
            ? { roleOverrides: record.roleOverrides as FlowRunDynamicWorkflowRequest['roleOverrides'] }
            : {};
        const authoringDraft = this.isRecord(record.authoringDraft)
            ? { authoringDraft: record.authoringDraft as unknown as FlowRunDynamicWorkflowRequest['authoringDraft'] }
            : this.isRecord(record.draft)
                ? { authoringDraft: record.draft as unknown as FlowRunDynamicWorkflowRequest['authoringDraft'] }
                : {};
    ...
    protected async startConversation(params: unknown): Promise<string> {
        ...
        try {
            ...
            // Fall back to a local conversation id when CyberVinci AI Providers is unavailable.
        }
        return `thread-${Date.now()}`;
    }
...
function normalizeFlowWorkflowSelector(value: string): string {
    return value
        .trim()
        .toLowerCase()
        .normalize('NFD')
