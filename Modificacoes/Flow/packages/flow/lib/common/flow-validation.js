...
function validateModelExecutionProfile(stateId, modelExecution, path, errors) {
    ...
    if (((_a = modelExecution.nativeReasoning) === null || _a === void 0 ? void 0 : _a.effort) !== undefined && !['none', 'low', 'medium', 'high'].includes(modelExecution.nativeReasoning.effort)) {
...
function validateStateReferences(stateId, state, path, stateIds, errors) {
    for (var _i = 0, _a = (state.waitFor || []).entries(); _i < _a.length; _i++) {
        var _b = _a[_i], index = _b[0], waitForStateId = _b[1];
