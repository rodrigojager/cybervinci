...
export type FlowAgenticRole = 'classifier' | 'planner' | 'executor' | 'candidate_generator' | 'critic' | 'judge' | 'verifier' | 'synthesizer' | 'repairer' | 'researcher' | string;
...
export declare const AGENCY_KERNEL_PROTOCOL_VERSION = "flow-kernel/stdio/v1";
export declare const AGENCY_KERNEL_WORKFLOW_VERSION = "flow.workflow/v1";
...
export type FlowKernelHostRequestType = 'execute_workload' | 'request_context_pack' | 'request_memory_write' | 'request_command_execution' | 'request_human_gate' | 'request_artifact_open';
export type FlowKernelHostCallbackType = 'workload_started' | 'workload_completed' | 'workload_failed' | 'artifact_created' | 'effect_recorded' | 'signal_recorded' | 'issue_recorded' | 'gate_approved' | 'gate_rejected' | 'clock_tick';
...
export type FlowRunExecutionMode = 'kernel_external' | 'kernel_simulated' | 'kernel_simulated_fallback_error' | 'capability_missing';
