...
exports.SISYPHUS_ULTRAWORK_COORDINATOR_PRESET_ID = exports.FLOW_PIPELINE_PRESET_VERSION = void 0;
exports.listBuiltInFlowPipelinePresets = listBuiltInFlowPipelinePresets;
exports.getBuiltInFlowPipelinePreset = getBuiltInFlowPipelinePreset;
exports.instantiateFlowPipelinePreset = instantiateFlowPipelinePreset;
exports.validateFlowPipelinePreset = validateFlowPipelinePreset;
var flow_validation_1 = require("./flow-validation");
exports.FLOW_PIPELINE_PRESET_VERSION = 'flow.pipeline-preset/v1';
exports.SISYPHUS_ULTRAWORK_COORDINATOR_PRESET_ID = 'sisyphus_ultrawork_coordinator';
var SISYPHUS_COORDINATOR_SYSTEM_PROMPT = [
...
var SISYPHUS_COORDINATOR_TASK_PROMPT = [
...
var SISYPHUS_ULTRAWORKER_SYSTEM_PROMPT = [
...
var SISYPHUS_ULTRAWORKER_TASK_PROMPT = [
...
var SISYPHUS_REVIEWER_SYSTEM_PROMPT = [
...
var SISYPHUS_REVIEWER_TASK_PROMPT = [
...
var SISYPHUS_COORDINATOR_AGENT_MARKDOWN = [
...
var SISYPHUS_ULTRAWORKER_AGENT_MARKDOWN = [
...
var SISYPHUS_REVIEWER_AGENT_MARKDOWN = [
    '# Sisyphus Reviewer',
    '',
    '## Role',
    '',
...
var BUILT_IN_PRESETS = [
    {
        id: exports.SISYPHUS_ULTRAWORK_COORDINATOR_PRESET_ID,
        ...
        version: exports.FLOW_PIPELINE_PRESET_VERSION,
        source: 'built-in',
...
function listBuiltInFlowPipelinePresets() {
    return clone(BUILT_IN_PRESETS);
}
function getBuiltInFlowPipelinePreset(id) {
    var preset = BUILT_IN_PRESETS.find(function (candidate) { return candidate.id === id; });
    return preset ? clone(preset) : undefined;
}
function instantiateFlowPipelinePreset(preset, options) {
    var workflow = clone(preset.workflow);
    var instantiated = __assign(__assign({}, workflow), { id: options.id, name: options.name, description: options.description || workflow.description });
    for (var _i = 0, _a = Object.entries(options.agentNodeOverrides || {}); _i < _a.length; _i++) {
        var _b = _a[_i], stateId = _b[0], override = _b[1];
        var state = instantiated.states[stateId];
        if ((state === null || state === void 0 ? void 0 : state.type) === 'agent') {
            instantiated.states[stateId] = applyAgentNodeOverride(state, override);
        }
    }
    return instantiated;
}
function validateFlowPipelinePreset(preset) {
    var errors = [];
    var warnings = [];
    ...
    if (preset.version !== exports.FLOW_PIPELINE_PRESET_VERSION) {
        errors.push({ code: 'preset.version.invalid', message: "Pipeline preset version must be ".concat(exports.FLOW_PIPELINE_PRESET_VERSION, "."), path: 'version' });
    }
    if (!preset.workflow || typeof preset.workflow !== 'object') {
        ...
        errors.push.apply(errors, workflowValidation.errors.map(function (error) { return prefixIssue(error, 'workflow'); }));
        warnings.push.apply(warnings, workflowValidation.warnings.map(function (warning) { return prefixIssue(warning, 'workflow'); }));
    }
...
function validateAgentMarkdown(preset, errors) {
    for (var _i = 0, _a = (preset.agentMarkdown || []).entries(); _i < _a.length; _i++) {
        var _b = _a[_i], index = _b[0], agent = _b[1];
        if (!agent.relativePath || agent.relativePath.split(/[\\/]+/).some(function (segment) { return segment === '..'; })) {
            errors.push({ code: 'preset.agent_markdown.path.invalid', message: "Agent markdown ".concat(index, " must use a safe relative path."), path: "agentMarkdown.".concat(index, ".relativePath") });
        }
        if (!agent.relativePath.endsWith('.md') && !agent.relativePath.endsWith('.markdown')) {
            errors.push({ code: 'preset.agent_markdown.extension.invalid', message: "Agent markdown ".concat(index, " must be a Markdown file."), path: "agentMarkdown.".concat(index, ".relativePath") });
        }
        if (typeof agent.content !== 'string' || agent.content.trim() === '') {
            errors.push({ code: 'preset.agent_markdown.content.required', message: "Agent markdown ".concat(index, " content is required."), path: "agentMarkdown.".concat(index, ".content") });
        }
    }
}
function applyAgentNodeOverride(state, override) {
    var _a, _b, _c, _d, _e, _f;
    return __assign(__assign({}, state), { provider: (_a = override.provider) !== null && _a !== void 0 ? _a : state.provider, modelExecution: (_b = override.modelExecution) !== null && _b !== void 0 ? _b : state.modelExecution, systemPrompt: (_c = override.systemPrompt) !== null && _c !== void 0 ? _c : state.systemPrompt, taskPrompt: (_d = override.taskPrompt) !== null && _d !== void 0 ? _d : state.taskPrompt, outputs: (_e = override.outputs) !== null && _e !== void 0 ? _e : state.outputs, deliverables: (_f = override.deliverables) !== null && _f !== void 0 ? _f : state.deliverables });
}
function prefixIssue(issue, prefix) {
    return __assign(__assign({}, issue), { path: issue.path ? "".concat(prefix, ".").concat(issue.path) : prefix });
}
