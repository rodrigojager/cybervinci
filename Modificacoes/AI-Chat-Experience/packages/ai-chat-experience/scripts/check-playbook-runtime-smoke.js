...
async function main() {
    ...
    states: [
      {
        ...
        tool: 'test.alwaysFail',
        args: {
          apiKey: 'sk-runtimefailure1234567890'
        },
        saveAs: 'failedResult'
      }
    ]
  ...
  const failedRunResult = await runtime.runPlaybookById(failingPlaybook.id, 'failure artifact request sk-runtimefailure1234567890');
  ...
  assert.equal(JSON.stringify(failedRun.failureArtifacts).includes('sk-runtimefailure1234567890'), false);
  assert.equal(JSON.stringify(failedRun).includes('sk-runtimefailure1234567890'), false);
