...
describe('FileContentIndexer', () => {
    ...
    it('extracts local PDF text only when opted in and redacts secrets before chunking', async () => {
        ...
        await fs.writeFile(path.join(workspacePath, relativePath), minimalPdf('Decision: never store the API key sk-test-secret1234567890 in reports.'), 'latin1');
        const file = fileFixture({
            id: 'file_pdf',
            relativePath,
        ...
        expect(withOptIn.map(chunk => chunk.content).join('\n')).not.to.contain('sk-test-secret1234567890');
    });
